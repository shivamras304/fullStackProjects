window.onload = function() {
  ...
  const portNumber = 3004
  const todoApiUrl = `http://127.0.0.1:${portNumber}/todos/`
  ...
  function drawTodoListAgain() {
    ...
    myTodoList.todos.forEach(function(todo) {
      flag = todo.done || flag;
      addToDoHtml(todo.text, todo.done, todo._id)
    })
  }
  ...
  function liClickEvent(event) {
    const todo = myTodoList.todos.filter(todo => todo._id == event.currentTarget.getAttribute("data-id"))[0]

    switch (event.target.getAttribute("data-target")) {
      case "todo-done":
        //XTODO: call PUT api here
        $.ajax({
          url: todoApiUrl + todo._id,
          method: "PUT",
          data: {
            text: todo.text,
            done: !todo.done
